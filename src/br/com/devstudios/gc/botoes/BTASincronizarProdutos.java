...
//				"WHERE X.REFERENCIA IS NOT NULL");
				sql.appendSql("SELECT CODPROD, DESCRPROD, MARCA, REFERENCIA, CODGRUPOPROD, CODVOL, EMPLOTE, USALOTE, DESCRLOTE, CASE WHEN USALOTE = 'L' THEN 'true' ELSE 'false' END AS VALIDATE_LOTE FROM("+
				"SELECT P.CODPROD,P.DESCRPROD,UPPER(NVL(I.MARCA,'SEM MARCA')) AS MARCA,"+
				"NVL((SELECT CODBARRA FROM TGFBAR WHERE CODVOL=P.CODVOL AND CODPROD=P.CODPROD AND ROWNUM=1),(SELECT CODBARRA FROM TGFVOA WHERE CODVOL=P.CODVOL AND CODPROD=P.CODPROD AND ROWNUM=1)) AS REFERENCIA,"+
				"P.CODGRUPOPROD,P.CODVOL,E.CODEMP AS EMPLOTE,NVL(E.TIPCONTEST,P.TIPCONTEST) AS USALOTE,"+
				"F_DESCROPC('TGFPRO', 'TIPCONTEST', NVL(E.TIPCONTEST,P.TIPCONTEST)) AS DESCRLOTE FROM TGFPRO P JOIN AD_INTMARCA I ON (I.ID=P.AD_MARCA) LEFT JOIN TGFPEM E ON (P.CODPROD=E.CODPROD) WHERE P.ATIVO='S' AND P.USOPROD IN ('"+usoprod+"')) X "+
				"WHERE X.REFERENCIA IS NOT NULL");
				...
				while (rs.next()) {
					...
					body.put("validate_lote", rs.getString("VALIDATE_LOTE"));
